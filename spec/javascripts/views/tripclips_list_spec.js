describe("views.TripclipsList", function() {
  ...
  describe("when the collection is fetched", function() {
    ...
    it("renders a list item with an item for each tripclip", function() {
      expect(items.length).to.equal(2);
    });

    it("renders each tripclip's name as a link", function() {
      expect(links.eq(0).text()).to.contain("zebra")
      expect(links.eq(1).text()).to.contain("chacha")
    });

    it("renders each tripclip's description", function() {
    ...
    describe("when a tripclip is selected", function() {
